...
namespace A031_StringBuilder
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            Console.WriteLine("{0} ({1} characters)", sb.ToString(), sb.Length); sb.Insert(5, "xyz ", 2);
            Console.WriteLine("{0} ({1} characters)", sb.ToString(), sb.Length);
            sb.Remove(5, 4);
            Console.WriteLine("{0} ({1} characters)", sb.ToString(), sb.Length);
            sb.Replace("xyz", "abc");
            ...
            time.Stop();
            Console.WriteLine("Using String concatenation: " + time.ElapsedMilliseconds + " milliseconds"); StringBuilder test1 = new StringBuilder();
            time.Reset();
            time.Start();
            for (int i = 0; i < 100000; i++)
            {
